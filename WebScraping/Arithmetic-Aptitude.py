...
def scrape_indiabix(query, query_number, start_page, end_page, output_folder="Arithmetic-Aptitude"):
    ...
    for i in range(start_page, end_page + 1):
        if i <= 9:
            url = f"https://www.indiabix.com/aptitude/{query}/{query_number}{i}"
        else:
            tens_place = i // 10
            i = i % 10 
            modified_query_number = f"{int(query_number) + tens_place:05d}"
            url = f"https://www.indiabix.com/aptitude/{query}/{modified_query_number}{i}"
        driver.get(url)
...
def process_html_files(query, input_folder="Arithmetic-Aptitude"):
...
def realtime_firebase(csv_path, query):
    db_path = f"Questions/Arithmetic-Aptitude/{query}"
...
if __name__ == "__main__":
    csv_file_path = os.path.join(script_dir, "Arithmetic-Aptitude.csv")
