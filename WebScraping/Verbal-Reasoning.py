...
def scrape_indiabix(query, query_number, start_page, end_page, output_folder="Verbal-Reasoning"):
    ...
    for i in range(start_page, end_page + 1):
        if i <= 9:
            url = f"https://www.indiabix.com/Verbal-Reasoning/{query}/{query_number}{i}"
        else:
            tens_place = i // 10
            i = i % 10 
            modified_query_number = f"{int(query_number) + tens_place:05d}"
            url = f"https://www.indiabix.com/Verbal-Reasoning/{query}/{modified_query_number}{i}"
        driver.get(url)
...
def process_html_files(query, input_folder="Verbal-Reasoning"):
...
def realtime_firebase(csv_path, query):
    db_path = f"Questions/Verbal-Reasoning/{query}"
    ...
    with open(csv_path, newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            try:
                ...
                print(f"Uploaded: {question_data}".replace("→", "->"))  # Replace Unicode arrow with ASCII
            except Exception as e:
                print(f"Error uploading row: {row} - {e}")

if __name__ == "__main__":
    csv_file_path = os.path.join(script_dir, "Verbal-Reasoning.csv")
